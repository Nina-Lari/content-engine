...
4. **Pick the composition.** Use the runbook's `Render:` line, or infer from the props shape: `clip` + `lines` + `literalImage` → `IdiomReel`; `clip` + `lines` → `ScenarioReel`; `coverImage` + `phrases` → `Cheatsheet`; `scenarioImage` + `options` → `Quiz`; `coverImage` + `cards` (+ `variant`) → `ConceptGuide`; `sceneImage` + `labels` → `VocabScene`; `cover` + `panels` + `lessons` → `ComicStory`.
...
2. If a slug was given, tell the user which composition to select (`ScenarioReel` / `IdiomReel` / `Cheatsheet` / `Quiz` / `ConceptGuide` / `VocabScene` / `ComicStory`) and that this post's props are in `remotion/props/<week>/{slug}.json` — they can load/paste those into the Studio props panel to preview the exact post.
